...
exports.asignarAtributoASku = exports.obtenerAtributosDeSku = exports.eliminarSku = exports.actualizarSku = exports.crearSku = exports.obtenerSkusPorProducto = void 0;
const database_1 = __importDefault(require("../config/database"));
const obtenerSkusPorProducto = async (req, res) => {
    const { idProducto } = req.params;
    let connection;
    try {
        connection = await database_1.default.getConnection();
        const result = await connection.execute(`SELECT ID_SKU, PRECIO, STOCK, ID_PRODUCTO
         FROM TBL_SKU
        WHERE ID_PRODUCTO = :idProducto
        ORDER BY ID_SKU`, [idProducto], { outFormat: database_1.default.OUT_FORMAT_OBJECT });
...
exports.obtenerSkusPorProducto = obtenerSkusPorProducto;
const crearSku = async (req, res) => {
    const { id_producto, precio, stock } = req.body;
    let connection;
    try {
        connection = await database_1.default.getConnection();
        const result = await connection.execute(`INSERT INTO TBL_SKU (ID_SKU, PRECIO, STOCK, ID_PRODUCTO)
...
const actualizarSku = async (req, res) => {
    ...
    try {
        connection = await database_1.default.getConnection();
        const result = await connection.execute(`UPDATE TBL_SKU
          SET PRECIO = :precio,
              STOCK  = :stock
        WHERE ID_SKU = :idSku`, { precio, stock, idSku }, { autoCommit: true });
...
exports.actualizarSku = actualizarSku;
const eliminarSku = async (req, res) => {
    const { idSku } = req.params;
    let connection;
    try {
        connection = await database_1.default.getConnection();
        await connection.execute(`DELETE FROM TBL_SKU_PRODUCTO WHERE ID_SKU = :idSku`, [idSku], { autoCommit: false });
        await connection.execute(`DELETE FROM TBL_CARRITO_ITEM WHERE ID_SKU = :idSku`, [idSku], { autoCommit: false });
        await connection.execute(`DELETE FROM TBL_PEDIDO_ITEM WHERE ID_SKU = :idSku`, [idSku], { autoCommit: false });
        const result = await connection.execute(`DELETE FROM TBL_SKU WHERE ID_SKU = :idSku`, [idSku], { autoCommit: false });
...
exports.eliminarSku = eliminarSku;
const obtenerAtributosDeSku = async (req, res) => {
    const { idSku } = req.params;
    let connection;
    try {
        connection = await database_1.default.getConnection();
      ...
      WHERE sp.ID_SKU = :idSku`, [idSku], { outFormat: database_1.default.OUT_FORMAT_OBJECT });
        res.json({ success: true, count: result.rows?.length || 0, data: result.rows });
    }
    catch (error) {
...
exports.obtenerAtributosDeSku = obtenerAtributosDeSku;
const asignarAtributoASku = async (req, res) => {
    const { idSku } = req.params;
    const { id_atributo } = req.body;
    let connection;
    try {
        connection = await database_1.default.getConnection();
        await connection.execute(`INSERT INTO TBL_SKU_PRODUCTO (ID_SKU, ID_ATRIBUTO)
       VALUES (:idSku, :idAtributo)`, { idSku, idAtributo: id_atributo }, { autoCommit: true });
...
exports.asignarAtributoASku = asignarAtributoASku;
//# sourceMappingURL=skuController.js.map