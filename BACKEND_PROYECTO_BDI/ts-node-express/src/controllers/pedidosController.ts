...
export const crearPedidoDesdeCarrito = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const idCarrito = (carritoRes.rows[0] as any).ID_CARRITO;
...
export const obtenerPedidosDeCliente = async (req: Request, res: Response) => {
  const { idCliente } = req.params;
  let connection: any;
