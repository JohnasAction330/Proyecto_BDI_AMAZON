...
export const obtenerCarritoCliente = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.rows && result.rows.length > 0) {
      carritoId = (result.rows[0] as any).ID_CARRITO;
    } else {
      const insert = await connection.execute(
        `INSERT INTO TBL_CARRITO (ID_CARRITO, FECHA_CREACION, ID_CLIENTE)
...
export const agregarItemCarrito = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (carritoRes.rows && carritoRes.rows.length > 0) {
      carritoId = (carritoRes.rows[0] as any).ID_CARRITO;
    } else {
      const insert = await connection.execute(
        `INSERT INTO TBL_CARRITO (ID_CARRITO, FECHA_CREACION, ID_CLIENTE)
         VALUES (SEQ_CARRITO.NEXTVAL, SYSDATE, :idCliente)
    ...
    const existeRes = await connection.execute(
      `SELECT CANTIDAD
         FROM TBL_CARRITO_ITEM
        WHERE ID_CARRITO = :idCarrito AND ID_SKU = :idSku`,
      { idCarrito: carritoId, idSku: id_sku },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );

    if (existeRes.rows && existeRes.rows.length > 0) {
      await connection.execute(
        `UPDATE TBL_CARRITO_ITEM
            SET CANTIDAD = CANTIDAD + :cantidad,
                FECHA_AGREGADO = SYSDATE
          WHERE ID_CARRITO = :idCarrito AND ID_SKU = :idSku`,
        { cantidad, idCarrito: carritoId, idSku: id_sku },
        { autoCommit: true }
      );
    } else {
      await connection.execute(
        `INSERT INTO TBL_CARRITO_ITEM (ID_SKU, ID_CARRITO, CANTIDAD, FECHA_AGREGADO)
         VALUES (:idSku, :idCarrito, :cantidad, SYSDATE)`,
        { idSku: id_sku, idCarrito: carritoId, cantidad },
        { autoCommit: true }
      );
    }
...
export const actualizarCantidadItem = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const carritoId = (carritoRes.rows[0] as any).ID_CARRITO;

    const result = await connection.execute(
      `UPDATE TBL_CARRITO_ITEM
          SET CANTIDAD = :cantidad
        WHERE ID_CARRITO = :idCarrito AND ID_SKU = :idSku`,
      { cantidad, idCarrito: carritoId, idSku },
      { autoCommit: true }
    );
...
export const vaciarCarrito = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await connection.execute(
      `DELETE FROM TBL_CARRITO_ITEM WHERE ID_CARRITO = :idCarrito`,
      [carritoId],
      { autoCommit: true }
    );
